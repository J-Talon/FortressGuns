...
public class AllocatorHalfSphereB extends Allocator<Collection<BlockPosition>, Tuple<Float, Vector>> {

    public AllocatorHalfSphereB(World nmsWorld, Vec3D position) {
     ...
     */
    @Override
    public Collection<BlockPosition> allocate(Tuple<Float, Vector> inputContext) {
        return getTraces(inputContext.b(), inputContext.a());
    }


    private Collection<BlockPosition> getTraces(Vector in, float radius) {

        org.bukkit.World bukkit = nmsWorld.getWorld();
        List<BlockPosition> dots = new ArrayList<>();
        for (Vector vector: getPositions(in,radius)) {
            ...
            while (travelled < radius) {
                ...
                dots.add(new BlockPosition(vectorX, vectorY, vectorZ));


                bukkit.spawnParticle(Particle.END_ROD, new Location(bukkit,vectorX, vectorY, vectorZ),1,0,0,0,0);
            }
        }
        return dots;
